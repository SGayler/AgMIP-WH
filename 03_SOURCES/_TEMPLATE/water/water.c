...
extern int WINAPI SingleCropCoeff(EXP_POINTER,float*,float*); //SG20160805
...
int WINAPI _loadds dllEPPotETPanCrop(EXP_POINTER)
{
 ...
 potevapotraPan(exp_p);
 //potevapotraPenMonteith(exp_p); //SG20160805
 if (pPl != NULL)
 {
  f1=CropCoeff(exp_p); 
  pWa->fPotETdt *= f1;
...
int WINAPI _loadds  dllEPPotETPenMonCrop(EXP_POINTER)
{
 ...
 if ((pPl != NULL)&& (PlantIsGrowing))
 {
  //if (NewDay(pTi)) 
  {
   ...
   SingleCropCoeff(exp_p,&x,&y); //SG20160805
   //pWa->fPotETDay *= x;
   //SG 20160728
 ...
 else //SG 20160728: bare soil
 {
   ...
   pWa->fPotETdt*=(float)0.25; //Kcb * ET0
 }
