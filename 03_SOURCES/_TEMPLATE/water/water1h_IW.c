...
int SetBC(EXP_POINTER)
{
 ...
 if (BotInF==TRUE)
 {
   ...
   //if(abs(hBotOld-hBot)>(float)1.e-8) MinStep=TRUE;
   	...
   	//SG20151001: Im ascii-mode wird fWaterTable nicht eingelesen (nur über DB!)
	//Für IW deshalb Grundwasserstand über Spalte 10 des Wetterfiles .xnw (=fPAR) einlesen
	//und hier auf pCl->pWeather->fWaterTable schreiben:
	if (pWa->iBotBC == (int)5)
	{
		if (pCl->pWeather->fWaterTable < 1e-9)
			pCl->pWeather->fWaterTable = pCl->pWeather->fPAR;
			
		hBot=-pCl->pWeather->fWaterTable + pSo->fDeltaZ/(float)2;
	}
	//original:
