...
int DevelopmentCheckAndPostHarvestManagement(EXP_POINTER)
  ...
  {  
    ...
    */
     ...
     fHeight = (float)200;
      ...
      if ((!lstrcmp(pPl->pGenotype->acCropCode,"RP\0")) || (!lstrcmp(pPl->pGenotype->acCropCode,"RY\0")))
      { //Raps
         fCut = (float)60;
         fHeight = (float)160;
      }
      
	   if (!lstrcmp(pPl->pGenotype->acCropCode,"SY\0"))
      { //Soja
         fCut = (float)15;
         fHeight = (float)65;
      }
  ...
  /*   if   (!lstrcmp(pPl->pGenotype->acCropName,"Ackersenf\0")||
          ...
          !lstrcmp(pPl->pGenotype->acCropName,"Ölrettich\0"))
     {
       if(pPl->pModelParam->cResidueCarryOff==0)
       {
         ...
         else
         {
          ...
          pCP->fCLitterSurf += (float)0.60*(pPl->pBiomass->fStovWeight+pPl->pBiomass->fGrainWeight) * (float)0.4;   
          pCP->fNLitterSurf += (float)0.60*(pPl->pBiomass->fStovWeight+pPl->pBiomass->fGrainWeight) * pPl->pPltNitrogen->fVegActConc;
          pCP->fCManureSurf += (float)0.40*(pPl->pBiomass->fStovWeight+pPl->pBiomass->fGrainWeight) * (float)0.4;   
          pCP->fNManureSurf += (float)0.40*(pPl->pBiomass->fStovWeight+pPl->pBiomass->fGrainWeight) * pPl->pPltNitrogen->fVegActConc;
         } 
       }
    } */
		if(pPl->pModelParam->cResidueCarryOff==0)
    {
		 ...
		 if(!lstrcmp(pPl->pGenotype->acCropCode,"WM\0")||
          !lstrcmp(pPl->pGenotype->acCropCode,"AL\0")||
          !lstrcmp(pPl->pGenotype->acCropCode,"LU\0")||
          !lstrcmp(pPl->pGenotype->acCropCode,"GC\0"))
         {
			 pCP->fCStandCropRes  += fCStandR; 
			 pCP->fCLitterSurf    += (float)0.60*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fCManureSurf    += (float)0.40*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fNStandCropRes  += fNStandR; 
			 pCP->fNLitterSurf    += (float)0.60*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
			 pCP->fNManureSurf    += (float)0.40*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
		  }
	
    else if(!lstrcmp(pPl->pGenotype->acCropCode,"WH\0")||
          !lstrcmp(pPl->pGenotype->acCropCode,"BA\0")||
          !lstrcmp(pPl->pGenotype->acCropCode,"S\0")||
		  !lstrcmp(pPl->pGenotype->acCropCode,"SY\0"))
	      {
			 pCP->fCStandCropRes  += fCStandR; 
			 pCP->fCLitterSurf    += (float)0.95*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fCManureSurf    += (float)0.05*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fNStandCropRes  += fNStandR; 
			 pCP->fNLitterSurf    += (float)0.95*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
			 pCP->fNManureSurf    += (float)0.05*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
		  }
		 		 
		 else if(!lstrcmp(pPl->pGenotype->acCropCode,"MZ\0"))
		 {
			 pCP->fCStandCropRes  += fCStandR; 
			 pCP->fCLitterSurf    += (float)0.85*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fCManureSurf    += (float)0.15*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fNStandCropRes  += fNStandR; 
			 pCP->fNLitterSurf    += (float)0.85*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
			 pCP->fNManureSurf    += (float)0.15*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
		  }

		 else if(!lstrcmp(pPl->pGenotype->acCropCode,"RP\0"))
		 {
			 pCP->fCStandCropRes  += fCStandR; 
			 pCP->fCLitterSurf    += (float)0.75*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fCManureSurf    += (float)0.25*(fCResidue - fCStandR)+pPl->pBiomass->fDeadLeafWeight*(float)0.4;
			 pCP->fNStandCropRes  += fNStandR; 
			 pCP->fNLitterSurf    += (float)0.75*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
			 pCP->fNManureSurf    += (float)0.25*(fNResidue - fNStandR)+pPl->pPltNitrogen->fDeadLeafNw;
		  }
	...
	//Root residues
	// SG: Oilradish (Gecros) - "stem" (= storage organ) should be below ground harvest residues. How to distribute to soil layers?
...
int WINAPI  dllEPPhotosynthesis_GECROS(EXP_POINTER)
  ...
  if NewDayAndPlantGrowing
    {
    //Wird nur berechnet wenn der Feldaufgang erreicht wurde
    if (pPl->pDevelop->iDayAftEmerg > 0) //SG 20160707
...
int WINAPI  dllEPOrganSenesz_GECROS(EXP_POINTER)
  {
	...
	//SG20170320 auskommentiert (weil manchmal freeAllocatedGECROSMemory2 und iGecros=0 schon im vorlezten Zeitschritt)
    ...
    if (pPl->pDevelop->iDayAftEmerg > 0)
		{
			...
			//leaves
			pCh->pCProfile->fCLitterSurf += pGecrosPlant->pGecrosCarbon->fCLeafLossR;
			pCh->pCProfile->fNLitterSurf += pGecrosPlant->pGecrosNitrogen->fNLeafLossR;
			...
			//if(pPl->pDevelop->fStageSUCROS>=pPl->pGenotype->fBeginSenesDvs)
			{
				...
				fDeadRootC = pGecrosPlant->pGecrosCarbon->fCRootLossR;
				fDeadRootN = pGecrosPlant->pGecrosNitrogen->fNRootLossR;
