...
extern int Germination(EXP_POINTER); //SG 20160706
...
int Emergence_GECROS(EXP_POINTER); //SG 20160706
...
extern float fparSINKBEET, fparSEEDW, fparSEEDNC, fparBLD, fparHTMX, fparMTDV, fparMTDR, fparPSEN;
extern float fparEFF;
extern float fparCFS;
...
//*/
	  ...
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
	  ...
	  //Sugar beet, Oilradish: pPltB->fStemWeight (= storage organ)

	  pGPltB->fLeafDeadWeight = (float)(WLVD); 
      pGPltB->fRootDeadWeight = (float)(WRTD);
	  pPltB->fDeadLeafWeight  = (float)(WLVD*10);//[g m-2] --> [kg ha-1]
	  pPltB->fDeadRootWeight  = (float)(WRTD*10);//[g m-2] --> [kg ha-1]
...
//    NRT    = (double)pPltN->fRootCont;
	  ...
	  pPltN->fGrainConc	      = (float)ONC;
	  pPltN->fFruitActConc    = (float)ONC;
...
//-----------------------------------------------------------------------------------------
      ...
      NUPT   = (double)pPltN->fActNUpt/10 + min(NDEM, NFIXR/TCP);//[kg N ha-1 d-1] --> [g N m-2 d-1]
     ...
     //SG20130304: verhindern, dass alles in Stängel und nichts in Blätter geht:
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
		  FCSST  = min(0.4,INSW(DS-(ESD+0.2), FLWCT/NOTNUL(DCSS), 0.));
	  else
      ...
      ///*
      ESD    = INSW(DETER, ESDI, 1.); //JR20170208 ZR, Vorverschiebung der Senescence?
...
//    LWLV   = min(WLV-1.E-5, LWLVM+REANOR(ESD-DS,LWLVM)*0.03*WLV);
	  ...
	  RCSST  = 12./44.*ASSA*    FCSH *    FCSST *YGV;//other crops rate
	  //SG 20180409: sugarbeet model J. Rabe - 'stem' is 'storage organ'
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
	  ...
	  RDCDTR = max(0.,(DCDTC-RCSST/YGV))-(FLWCT-min(DCDTC,DCST));//other crops rate
	  //SG 20180409: sugarbeet model J. Rabe - 'stem' is 'storage organ'
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
	      RDCDTR = max(0.,(DCDTC-RCSST/YGO))-(FLWCT-min(DCDTC,DCST));//sugar beet rate

      RWLV   = RCLV / CFV;
	  RWST   = RCSST/ CFV + RCRVS/0.444;//other crops
	  //SG 20180409: sugarbeet model J. Rabe - 'stem' is 'storage organ'
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
...
//		EFV = abspowerDBL(CVU,(double)5)/(abspowerDBL((double)22.5,(double)5)+abspowerDBL(CVU,(double)5)); 
	  ...
	  if(fparVERN == (float)0) EFV = (double) pPl->pDevelop->fVernEff; //vernalization following CERES

	  if(fparVERN < (float)0) EFV = (double) 1; // no vernalization required (spring crop)
...
//	  WCUL = (WUL+WCMIN*10.*RD)/10./NOTNUL(RD);
      ...
      pWa->pEvap->fPotDay              = (float)PESOIL;// EP 20130731 wird in "balance.c" berechnet
      pWa->pEvap->fActDay              = (float)AESOIL;// EP 20130731 wird in "balance.c" berechnet
	 //SG 20140216 AgMIP: "CIMMYT"/"Global T"
      pWa->pEvap->fActDay              = (float)0.0;
	  ...
	  pWa->pEvap->fActR                = (float)AESOIL; // EP 20130731 wird in "water1h.c"(HYDRUS) berechnet
	  //SG 20140216 AgMIP: "CIMMYT"/"Global T"
      pWa->pEvap->fActR              = (float)0.0;
	  ...
	  pWa->fPotETDay                   = (float)(PESOIL+PTCAN);// EP 20130731 wird in "balance.c" berechnet
	  ...
	  pWa->fPotETdt                    = pWa->fPotETDay * pTi->pTimeStep->fAct;//EP 20130731
          pPl->pPltWater->fPotTranspdt     = (float)PTCAN * pTi->pTimeStep->fAct; //EP 20130731
          pPl->pPltWater->fActTranspdt     = (float)ATCAN * pTi->pTimeStep->fAct;//EP 20130731
...
double kdiff(double vLAI, double BL, double SCP)
      {
        ...
        double KB15,KB45,KB75,KDP;
		 ...
		 //SG20191211: only for AgMIP-WH Phase_4 CIMMYT#2 --> Baca-DH
		 KDP = 	KDP*1.1;	//10% increase in k_dif
 ...
 //     DEJMAX = (double)152044.; //energy of deactivation for JMAX (J/mol) - Müller et al., Ecol. Modelling 183 (2005), 183–210
        ...
        else
      {
       ...
       ALPHA2 = ((double)1.-FCYC)/((double)1.+((double)1.-FCYC)/PHI2M);
	   	//SG20191211: only for AgMIP-WH Phase_4 CIMMYT#2 --> Baca-DH
		ALPHA2 = ALPHA2*1.34;	//34% increase in LUE
  ...
  int   WINAPI _loadds Init_GECROS(EXP_POINTER)
     {
      ...
      VERN  =  (double)fparVERN; // VERN = 1. vernalization model SPASS; 
	                             // VERN = 0. vernalization model CERES; 
								 // VERN =-1. without veralization (spring crop)
	  ...
	  //SG 20180410: additional parameters for sugar beet
	  SINKBEET = (double)fparSINKBEET;
	  EFF = (double)fparEFF;
	  CFS = (double)fparCFS;
	  ...
	  FPRO   = (double)6.25*SEEDNC; //other crops
	  //SG 20180409: sugarbeet model J. Rabe - 'stem' is 'storage organ'
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
		  FPRO   = (double)6.25*STEMNC; //sugar beet
...
//AP20160616
	  NLVI   = min(FNRSH * NPL * SEEDW * EG * SEEDNC, LNCI * CLVI/CFV);
	  NRTI   = NPL * SEEDW * EG * SEEDNC - NLVI;

	  //SG 20180409: sugarbeet model J. Rabe - accounting for small seed number and low seed weights
	  if((!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))||(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"OR")))
	  {
		CLVI   = NPL * SEEDW * EFF * CFS * EG * FCRSH;
		CRTI   = NPL * SEEDW * EFF * CFS * EG * (1.-FCRSH);

		NLVI   = LNCI* CLVI/CFV;
		NRTI   = NPL * SEEDW * EFF * EG * LNCI * FCRSH/FNRSH - NLVI;
	  }
...
int   PlantParametersGECROS(LPSTR species,LPSTR variety)
{
  ...
  extern float fparSINKBEET,fparEFF,fparCFS; //for sugar beet
  extern float fparPMEH,fparPMES,fparESDI,fparWRB;
	  ...
	  if (pDev->fStageSUCROS < 0)
	  {
		  ...
		  else //SG 20160706: Berechnung von Keimung (Germination) und Feldaufgang (Emergence) in Abhängigkeit von Umwelteinflüssen nach CERES
		  {
			  if (pDev->fStageSUCROS < -0.51)
			  {
				  ...
				  iGermination = Germination(exp_p); //aus Ceres.c
		      
				 if (iGermination < 1)
				 {
					 pDev->fDevR = (float)0.0;//rate
					 return -1;
				 }
				 else
				 {
...
//-------------------------------------------------------------------------------------
      ...
      //%---rate of LAI driven by nitrogen during juvenile phase    
      if ((LAIC < (double)1)&&(DS < (double)0.5))
      //SG 20180410: sugarbeet model by J.Rabe
	  //if(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"SB"))
		 // RLAI  = (SLNB*RNLV-NLV*RSLNB)/SLNB/(SLNB+KN*NLV);
	  //else
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//      PotentialNitrogenUptake_GECROS
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
int WINAPI _loadds PotentialNitrogenUptake_GECROS(EXP_POINTER)
   {
      ...
      double DELT = (double)1;
	//  double DSCRIT = (double)0.35;//0.25;
...
//    NRT    = (double)pPltN->fRootCont/10.0;
   ...
   //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	// SG20140312:
	// Vorschlag von Joachim Ingwersen zur Simulation der Winterung
    if(!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"WH")            // here we have to distinguish between
				||!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"BA")) // winter and summer crops --> DB!!!
	{
	...
	if (!lstrcmp((LPSTR)pPl->pGenotype->acCropCode,(LPSTR)"RP"))
	{
	//	if (DS < 0.25) // DSCRIT = 0.25
		if ((fparVERN > -0.1) && (DS < 0.3)) // DSCRIT = 0.25
    		NDEM   = INSW(SLNMIN-SLN+1.E-5, min(NUPTX,0.01*NDEMAD), 0.);
		else
    		NDEM   = INSW(SLNMIN-SLN+1.E-5,  min(NUPTX,NDEMAD), 0.);
	}
...
///*
  ...
  int Emergence_GECROS(EXP_POINTER)
  {
	...
	//=========================================================================================
		...
		{
			if(pCl->pWeather->fTempAve < fTbase_Emerg)
				fDTT = 0;
			else
				fDTT = pCl->pWeather->fTempAve - fTbase_Emerg;

			fEmergValue += fDTT/fP9;
		}

		if (fEmergValue < (float) 1.0)
			iEmergence = -1;
		else
			iEmergence = 1;
